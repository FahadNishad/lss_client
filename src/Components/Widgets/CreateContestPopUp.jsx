...
const CreateContestPopUp = ({ isOpen, setIsOpen, number }) => {
  ...
  const [contestData, setContestData] = useState({
    ContestName: 'Super Bowl Contest', // Default value for ContestName
  ...
  return (
    <>
      ...
      <div
        ...
        className={`fixed top-0 right-0 h-full md:w-[25%] overflow-y-auto bg-white shadow-lg z-50 transition-transform duration-300 ease-in-out transform ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
        <div className="flex justify-between items-center p-4 bg-gray-100 border-b border-gray-200">
          ...
          <button className="text-gray-600 text-2xl" onClick={toggleSidebar}>&times;</button>
        </div>
        ...
        <div className="p-6">
          ...
          <img style={{ display: number === "100" || number === null ? '' : "none" }} src={img} alt="Contest grid" className="w-full mb-4" />
          <h3 style={{ display: number === null ? '' : 'none' }} className="text-xl font-semibold mb-4">Ready to Begin?</h3>
          <p className="text-sm text-gray-600 mb-6">The process takes 15 seconds and is free! Just fill out the form below and hit the button!</p>
